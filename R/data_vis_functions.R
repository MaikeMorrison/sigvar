...
Q_checker <- function(Q, K, rep) {
  ...
  if (K > ncol(Q)) {
    stop("K is larger than the number of columns in the provided matrix.")
  }
...
#'
#' This function enables graphical visualization of a matrix of compositional data.
...
#' sig_activity <- dplyr::mutate(data.frame(sig_activity), dplyr::across(
#'   c(SBS1, SBS2, SBS3),
#'   as.numeric
#' ))
#'
  ...
  if (facet_true) {
    ...
    sig_activity_present <- sig_activity_sigs %>%
      ...
      dplyr::summarise(dplyr::across(
        dplyr::all_of(signatures),
        function(x) mean(x > threshold)
      )) %>%
      tidyr::pivot_longer(
        cols = dplyr::all_of(signatures), names_to = "Signature",
        values_to = "Proportion_present"
      )
  } else {
    sig_activity_sigs <- cbind(
      data.frame("group" = sig_activity[[group]]),
      (if (normalized) {
