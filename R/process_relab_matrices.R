...
# when generating bootstrap replicates of matrices
...
arrange_categories <- function(relab_matrix, arrange, K = NULL, group = NULL, time = NULL) {
  if (arrange == FALSE) {
    ...
    clustermeans <- colMeans(relab_matrix_clean) %>%
      sort() %>%
      rev()
  ...
  if (!is.null(group)) {
    relab_matrix_clean <- relab_matrix_clean %>%
      data.frame() %>%
      dplyr::mutate(group = relab_checker_out$group, .before = 1)
  }

  if (!is.null(time)) {
    relab_matrix_clean <- relab_matrix_clean %>%
      data.frame() %>%
      dplyr::mutate(time = relab_checker_out$time, .before = 1)
  }
