...
test_that("mean normalized works with facet", {
  expect_warning(plot_dots(
    sig_activity = sig_activity, group = "Country",
    facet = "Incidence_Level", K = 43
  ))
  expect_no_warning(plot_dots(
    sig_activity = ESCC_sig_activity, group = "Country",
    facet = "Incidence_Level", K = 43
  ))
})
...
test_that("median normalized works with facet", {
  expect_warning(plot_dots(
    sig_activity = sig_activity, median = TRUE, group = "Country",
    facet = "Incidence_Level", K = 43
  ))
  expect_no_warning(plot_dots(
    sig_activity = ESCC_sig_activity, median = TRUE, group = "Country",
    facet = "Incidence_Level", K = 43
  ))
})
