...
output:
  BiocStyle::html_document:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Signature Variability Analysis of the mutational signatures of driver alterations with sigvar}
...
```{r }
data(Sherlock_LCINS_SBS, package = "sigvar")
...
```{r }
data(Sherlock_LCINS.metadata, package = "sigvar")

Sherlock_LCINS_SBS_res <- bind_cols(
  prob.TP53 = prob_Sherlock_TP53_drivers,
...
```{r }
ggplot(data = Sherlock_LCINS_SBS_res, aes(y = prob.TP53, x = GS)) +
  geom_point() +
  ...
  ylab("Probability of TP53 driver alteration") +
  theme_classic()
```
...
```{r}
ggplot(tibble(
  Type = str_extract(names(TP53_driver_prop), "[ACGT]>[ACGT]"),
  Subtype = paste0(substr(names(TP53_driver_prop), 1, 1), substr(names(TP53_driver_prop), 3, 3), substr(names(TP53_driver_prop), 7, 7)),
  probability = TP53_driver_prop
), aes(x = paste0(Type, Subtype), y = probability, fill = Type)) +
  geom_col() +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
  xlab("Mutation type") +
  ylab("Proportion of drivers")
```
