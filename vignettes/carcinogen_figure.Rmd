...
output:
  BiocStyle::html_document:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{sigvar analysis of samples with varying carcinogen exposure}
...
riva_relab_counts <- riva_relab %>%
  group_by(Tissue, chemical) %>%
  summarise(n = n()) %>%
  filter(n > 1)
...
data(zhang_sig_activity, package = "sigvar")
data(zhang_sig_similarity, package = "sigvar")
...
```{r}
data(MESOMICS_CN_SBS, package = "sigvar")
data(COSMIC3.1_CN, package = "sigvar")
...
meso <- bind_cols(
  ...
  na.omit() %>%
  mutate(
    Asbestos_exposure_formatted = ifelse(Asbestos_exposure == "Exposed",
      "Exposed to\nasbestos",
      "Unexposed to\nasbestos"
    ),
    .before = 2
...
```{r}
data(radiation_sigs_morton, package = "sigvar")
data(radiation_sigs_morton_cossim, package = "sigvar")

morton <- radiation_sigs_morton
morton <- morton %>%
  mutate(radiation_dose_groups = factor(
    case_when(
      morton$Radiation_dose >= 250 ~ "Exposed to\nradiation (≥250mGy)",
...
carcinogens_within <- rbind(
  data.frame(
    group = zhang$passive_smoking,
  ...
  data.frame(
    group = meso$Asbestos_exposure_formatted,
    within = apply(meso, 1, het, K = 7, S = meso_cossim),
    data = meso_label
  ),
  data.frame(
    group = morton$radiation_dose_groups,
...
group_labels <- group_labels %>% mutate(
  max_mean_within = max(mean_within) + .18,
...
carcinogens_plot <-
  ...
  geom_richtext(aes(x = -0.18, y = across, label = data),
    data = data_labels,
    hjust = 0,
  ...
  geom_violin(
    aes(
      x = within, y = across, fill = group,
      group = paste0(group, data)
    ),
    color = NA,
  ...
  geom_path(
    data = group_labels, mapping = aes(x = mean_within, y = across, group = data),
    linewidth = 1,
  ...
  coord_cartesian(xlim = c(-0.145, 0.8))
carcinogens_plot
```
...
arrows_df <- data.frame(
  xstart = c(
    0.08, 0.08, within_center - 0.2,
    within_center + 0.2
  ),
  xend = c(
    0.08, 0.08, within_center - 0.42,
    within_center + 0.42
  ),
  ystart = c(
    across_center - 0.2, across_center + 0.2,
    0.018, 0.018
  ),
  yend = c(
    across_center - 0.42, across_center + 0.42,
    0.018, 0.018
  ),
  xlab = c(
    -0.01, -0.01, within_center - 0.31,
    within_center + 0.31
  ),
  ylab = c(
    across_center - 0.31, across_center + 0.31,
    -0.025, -0.025
  ),
...
carcinogens_arrows <- base +
  annotation_custom(
    grob = ggplotGrob(carcinogens_plot +
