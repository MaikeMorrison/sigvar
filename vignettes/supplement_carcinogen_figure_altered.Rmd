...
riva_within = riva_relab_norm %>% dplyr::select(chemical, Tissue) %>%
...
for(chem in setdiff(chems, "Spontaneous")){
  ...
  for(tissue in unique(riva_chem$Tissue)){
    riva_bootstrap_list_greater[[i]] = sigvar::sigboot(sig_activity = riva_chem_spontaneous %>% dplyr::filter(Tissue == tissue),
                                                       K = 11, S = riva_cossim,
                                                       group = c("chemical", "Tissue"), 
...
chems_ordered = riva_within_sig2 %>% dplyr::filter(Tissue == "liver") %>%
  arrange(-mean_within) %>%
...
chemical_counts_liver <- riva_relab_norm %>% dplyr::filter(Tissue=="liver") %>% group_by(chemical) %>% count()
chemical_counts_lung <- riva_relab_norm %>% dplyr::filter(Tissue=="lung") %>% group_by(chemical) %>% count()
...
chems_ordered = riva_within_sim_sig2 %>% dplyr::filter(Tissue == "liver") %>%
  arrange(-mean_within) %>%
  dplyr::select(chemical_bold) %>% unique %>% unlist
