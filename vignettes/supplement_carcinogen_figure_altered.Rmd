...
output:
  BiocStyle::html_document:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Carcinogen exposure is associated with increased diversity of signature activity within tumors}
...
riva_relab <- mutsig_carcinogens_mice_SBS %>%
  left_join(chem_abbreviations) %>%
...
riva_var <- sigvar(
  sig_activity = riva_relab_norm,
  K = 11, S = riva_cossim,
  group = c("chemical", "Tissue")
) %>%
  `colnames<-`(c("chemical", "Tissue", "group", "across", "mean_within"))

riva_within <- riva_relab_norm %>%
  dplyr::select(chemical, Tissue) %>%
...
for (chem in setdiff(chems, "Spontaneous")) {
  riva_chem_spontaneous <- dplyr::filter(riva_relab_norm, chemical %in% c("Spontaneous", chem))
  riva_chem <- dplyr::filter(riva_relab_norm, chemical == chem)
  for (tissue in unique(riva_chem$Tissue)) {
    riva_bootstrap_list_greater[[i]] <- sigvar::sigboot(
      sig_activity = riva_chem_spontaneous %>% dplyr::filter(Tissue == tissue),
      K = 11, S = riva_cossim,
      group = c("chemical", "Tissue"),
...
data(mutsig_carcinogens_mice_bootstrap_p_vals, package = "sigvar")
...
bold_list <- sig2 %>%
  ...
  dplyr::select(chemical, chemical_bold) %>%
  distinct() %>%
  dplyr::filter(chemical != "Spontaneous") %>%
...
chems_ordered <- riva_within_sig2 %>%
  dplyr::filter(Tissue == "liver") %>%
  arrange(-mean_within) %>%
  dplyr::select(chemical_bold) %>%
  unique() %>%
  unlist()
...
riva_1d <- ggplot(riva_within_sig2, aes(y = chemical_ordered, x = within)) +
  ...
  geom_vline(aes(xintercept = mean_within),
    data = riva_within_sig2 %>% dplyr::filter(chemical == "Spontaneous"),
    linewidth = 1, color = "#3A488AFF", alpha = 0.7
  ) +
  geom_violin(
    fill = "#3A488AFF", color = NA, width = 0.9, alpha = 0.7,
    data = riva_within_sig2 %>% dplyr::filter(chemical == "Spontaneous")
  ) +
  geom_violin(aes(fill = significance),
    color = NA, width = 1.3, alpha = 0.7,
    data = riva_within_sig2 %>% dplyr::filter(chemical != "Spontaneous")
  ) +
...
base_within_only <- ggplot(data.frame(
  xstart = riva_center + c(-0.15, 0.15),
...
chemical_counts_liver <- riva_relab_norm %>%
  dplyr::filter(Tissue == "liver") %>%
  group_by(chemical) %>%
  count()
chemical_counts_lung <- riva_relab_norm %>%
  dplyr::filter(Tissue == "lung") %>%
  group_by(chemical) %>%
  count()

# number of samples per substance group
chemical_counts_liver <- arrange(chemical_counts_liver[-1, ], "chemical")
chemical_counts_lung <- arrange(chemical_counts_lung[-1, ], "chemical")

liver_spont <- riva_relab_norm %>% dplyr::filter(Tissue == "liver", chemical == "Spontaneous")
lung_spont <- riva_relab_norm %>% dplyr::filter(Tissue == "lung", chemical == "Spontaneous")
...
riva_within_sim_liver <- arrange(riva_within %>% dplyr::filter(Tissue == "liver", chemical != "Spontaneous"), "chemical")
riva_within_sim_lung <- arrange(riva_within %>% dplyr::filter(Tissue == "lung", chemical != "Spontaneous"), "chemical")
...
riva_within_sim <- rbind(riva_within_sim_liver, riva_within_sim_lung, riva_within %>% dplyr::filter(Tissue == "liver", chemical == "Spontaneous"), riva_within %>% dplyr::filter(Tissue == "lung", chemical == "Spontaneous"))
```
...
lpvals <- c(
  t.test(riva_within_sim[1:4, 3], riva_within_sim[148:158, 3])$p.value, # tcp
  ...
  t.test(riva_within_sim[143:147, 3], riva_within_sim[159:170, 3])$p.value
) # vdc lung
...
chems_ordered <- riva_within_sim_sig2 %>%
  dplyr::filter(Tissue == "liver") %>%
  arrange(-mean_within) %>%
  dplyr::select(chemical_bold) %>%
  unique() %>%
...
riva_1d <- ggplot(riva_within_sim_sig2, aes(y = chemical_ordered, x = within)) +
  geom_point(aes(x = mean_within, y = chemical_ordered), size = 3, alpha = 0) +
  geom_vline(aes(xintercept = mean_within),
    data = riva_within_sim_sig2 %>% dplyr::filter(chemical == "Spontaneous"),
    linewidth = 1, color = "#3A488AFF", alpha = 0.7
  ) +
  geom_violin(
    fill = "#3A488AFF", color = NA, width = 0.9, alpha = 0.7,
