...
data(rcc_sig_activity, package = "sigvar")
...
ESCC_sig_activity_long <- tidyr::pivot_longer(ESCC_sig_activity,
  ...
  mutate(
    Sig_Type = ifelse(stringr::str_detect(Signature, "SBS"), "SBS",
  ...
  group_by(Sample) %>%
  mutate(Rel_Abundance_all = Abundance / sum(Abundance)) %>%
...
escc_scatter <-
  ...
  geom_point(aes(
    color = incidence # `ASR (World)`, shape = incidence
  ), size = 3, alpha = 0.7) +
...
summary(lm(
  across_sample_heterogeneity ~ mean_within_sample_diversity,
  escc_stats
))
...
cor.test(
  x = escc_stats$across_sample_heterogeneity,
...
cor.test(
  x = escc_stats$mean_within_sample_diversity,
  y = as.numeric(escc_stats$incidence == "High incidence"), method = "spearman"
)
```
...
```{r}
omit_sigs <- which(colSums(dplyr::select(
  ESCC_sig_activity,
  all_of(all_sigs)
) > 0) /
...
meanplot_ALL_df <- ESCC_sig_activity %>%
  mutate(
    Incidence_Level = ifelse(Incidence_Level == "High",
      "High-incidence countries",
      "Low-incidence countries"
    ) %>%
      factor(ordered = TRUE, levels = c(
        "Low-incidence countries",
        "High-incidence countries"
      )),
...
escc_dot_plot <- plot_dots(
  sig_activity = meanplot_ALL_df,
  K = 43,
  group = "Country",
  max_dotsize = 3,
  pivot = TRUE,
...
```{r}
all_sigs_ordered <- ESCC_sig_activity_long %>%
  group_by(Signature) %>%
...
japan_relab <- plot_signature_prop(japan, K = 43, arrange = "both") +
  scale_fill_manual(
    values = all_sig_pal, breaks = j_sigs_ordered,
    name = "Signature"
  ) +
  scale_color_manual(
    values = all_sig_pal, breaks = j_sigs_ordered,
...
tanzania_relab <- plot_signature_prop(tanz, K = 43, arrange = "both") +
  scale_fill_manual(
    values = all_sig_pal, breaks = t_sigs_ordered,
    name = "Signature"
  ) +
  scale_color_manual(
    values = all_sig_pal, breaks = t_sigs_ordered,
...
plot_legend <- ggplot() +
  ...
  scale_fill_manual(
    values = all_sig_pal, breaks = jt_sigs_ordered[1:10],
    name = "Signature"
  )
...
rcc_stats <- sigvar(
  sig_activity = rcc_clean, K = 28, group = "Country" # , S = S
...
cor.test(
  x = rcc_stats$mean_within_sample_diversity,
  y = rcc_stats$ASR, alternative = "greater"
)
cor.test(
  x = rcc_stats$across_sample_heterogeneity,
...
rcc_scatter <- ggplot(rcc_stats, aes(
  x = mean_within_sample_diversity,
  y = across_sample_heterogeneity
)) +
  ...
  scale_color_gradientn(
    colors = PNWColors::pnw_palette("Bay", 6),
...
rcc_dot_plot <- plot_dots(
  sig_activity = meanplot_rcc,
  K = 28,
  group = "Country",
  max_dotsize = 3,
  pivot = TRUE
...
plot_legend_rcc <- ggplot() +
  geom_raster(aes(
    x = 1:10, y = 2:11,
    fill = lr_sigs_ordered[1:10]
  )) +
  scale_fill_manual(
    values = all_sig_pal, breaks = lr_sigs_ordered[1:10],
    name = "Signature"
  )
...
ylims <- c(
  min(
    rcc_stats$across_sample_heterogeneity,
...
xlims <- c(
  min(
    rcc_stats$mean_within_sample_diversity,
...
escc_inset <- escc_scatter +
  ...
  ylim(
    min(escc_stats$across_sample_heterogeneity),
    max(escc_stats$across_sample_heterogeneity)
  ) +
  xlim(
    min(escc_stats$mean_within_sample_diversity),
...
escc_arrows <- base +
  annotation_custom(
    grob = ggplotGrob(escc_scatter_inset +
...
panels <- cowplot::plot_grid(
  ...
  rcc_dot_plot_arrow + theme(plot.margin = unit(
    c(t = 0, l = 0.8, b = 0, r = 0),
    "lines"
  )),
